...
def save_model_history(history, history_path):    
    ...
    df_history.to_csv(history_path+"/AD_MCI_1365_vgg-bilstm-bilstm_1_16_08_21_axial_slices.csv", index=False)
    return    
...
def fit_generator(model, training_generator, validation_generator, checkpoint_path):    
    ...
    mc = tf.keras.callbacks.ModelCheckpoint('/media/iitindmaths/Seagate_Expansion_Drive/Bup_Backup/SPM/alzheimers-disease/MCI_vs_AD/model_checkpoints/AD_MCI_1365_vgg-bilstm-bilstm_1_16_08_21_axial_slices_{epoch:08d}.h5', 
...
lstm = LSTM(50, return_sequences=True)(flattened_model)
lstm = Flatten()(lstm)
...
# dense1 = Dense(10,activation='relu')(lstm)
# dense1 = Dense(1,activation='sigmoid')(dense1)
# vgg_lstm = Model(inputs=full_model.input, outputs=dense1) #val data - 99.32
