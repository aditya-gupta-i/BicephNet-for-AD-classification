print("********************************************************************")
print("CN vs AD VGG Conv LSTM Axial now running")
...
from tensorflow.keras.layers import Dropout, Flatten, Dense,Lambda, Conv2D, MaxPooling2D, ConvLSTM2D, Input, TimeDistributed
...
def save_model_history(history, history_path):    
    ...
    df_history.to_csv(history_path+"/MCI_AD_1365_vgg-clstm_1_16_08_21_axial_slices.csv", index=False)
    return    
...
def fit_generator(model, training_generator, validation_generator, checkpoint_path):    
    ...
    mc = tf.keras.callbacks.ModelCheckpoint('/media/iitindmaths/Seagate_Expansion_Drive/Bup_Backup/SPM/alzheimers-disease/MCI_vs_AD/model_checkpoints/MCI_AD_1365_vgg-clstm_1_16_08_21_axial_slices_{epoch:08d}.h5', 
...
vgg_l2 = Conv2D(128, (1,1), padding='same',activation='relu')(vgg_l1)
vgg_l3 = Conv2D(64, (1,1), padding='same',activation='relu')(vgg_l2)
...
clstm = ConvLSTM2D(filters = 32, kernel_size=(3,3), return_sequences=True, activation='relu',padding='same')(full_model.output)
clstm = ConvLSTM2D(filters = 32, kernel_size=(3,3), activation='relu',padding='same',return_sequences=True)(clstm)
flat = Flatten()(clstm)
